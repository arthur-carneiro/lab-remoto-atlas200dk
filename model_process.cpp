...
void ModelProcess::OutputModelResult()
{
    ...
    ifstream arquivo("../out/class_index.json");
    json classe;// = json::parse(arquivo);
    arquivo >> classe;
	...
	A = classe[to_string(it->second)][1];
           //INFO_LOG("top %d: index[%d] value[%lf]", cnt, it->second, it->first*100);
	INFO_LOG("top %d: index[%d] - value[%.2lf%%]", cnt, it->second,  it->first*100);	
	cout << A << endl;	
        }
        if (!g_isDevice) {
            ret = aclrtFreeHost(outHostData);
